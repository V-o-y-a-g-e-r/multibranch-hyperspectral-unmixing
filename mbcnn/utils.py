...
def subsample_test_set(data: Dict, test_size: int):
    ...
    assert data[enums.Dataset.DATA].shape[SAMPLES_DIM] >= test_size and \
        ...
        'The test size is not sufficient to subsample:\n' \
        f'Current size available: {data[enums.Dataset.DATA].shape[SAMPLES_DIM]}\n' \
        f'Designed set size: {test_size}\n' \
        f'Decrease either the --train-size or the --sub-test-size arguments.'
...
def build_data_dict(train_x, train_y, val_x,
                    val_y, test_x, test_y, name) -> Dict:
    """
    ...
    train_mean, train_std = np.mean(train_x), np.std(train_x)
    data_dict[enums.DataStats.MIN] = train_min
    data_dict[enums.DataStats.MAX] = train_max
    data_dict[enums.DataStats.MEAN] = train_mean
    data_dict[enums.DataStats.STD] = train_std
    ...
    data_dict[enums.Dataset.NAME] = os.path.basename(name).split('.')[0]
    return data_dict
