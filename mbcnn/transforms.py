...
from mbcnn.models import unmixing_pixel_based_dcae, \
    unmixing_cube_based_dcae, unmixing_cube_based_cnn, \
    unmixing_pixel_based_cnn, \
    multiple_feature_learning_model_rezidual, \
    multiple_output_feature_learning_model, \
    multiple_feature_learning_model_base, \
    multiple_feature_learning_model_feature_extraction, \
    multiple_feature_learning_model_feature_extraction_rezidual, \
    unmixing_linear, unmixing_svr, unmixing_deep_cnn
...
class Standardize(BaseTransform):
    ...
    def __call__(self, sample: np.ndarray, label: np.ndarray) -> List[
        ...
        :return: List containing the normalized sample and the class label.
        """
        return [(sample - self.mean) / self.std, label]
...
class ExtractCentralPixelSpectrumTransform(BaseTransform):
    ...
    def __call__(self, sample: np.ndarray,
                 label: np.ndarray) -> List[np.ndarray]:
        """"
        Transform the samples for unsupervised unmixing problem.
        ...
        if self.neighborhood_size is not None:
            ...
            sample = np.squeeze(sample[:, central_index, central_index])
        else:
...
class Flatten(BaseTransform):
    ...
    def __call__(self, sample: np.ndarray,
                 label: np.ndarray) -> List[np.ndarray]:
        """"
        Transform the samples for unmixing problem
        utilizing all features as flatten array.
...
UNMIXING_TRANSFORMS = {
    ...
    multiple_feature_learning_model_base.__name__: [SpectralTransform],
    multiple_feature_learning_model_rezidual.__name__: [SpectralTransform],
    multiple_feature_learning_model_feature_extraction.__name__: [
        SpectralTransform],
    multiple_feature_learning_model_feature_extraction_rezidual.__name__: [
        SpectralTransform],

    multiple_output_feature_learning_model.__name__: [SpectralTransform],

    unmixing_linear.__name__: [ExtractCentralPixelSpectrumTransform],
    unmixing_svr.__name__: [Flatten],
    unmixing_deep_cnn.__name__: [SpectralTransform]
}
